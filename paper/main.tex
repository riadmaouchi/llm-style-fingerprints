...
modest 36.9\% accuracy: rewrites are by design closer to the original
...
\paragraph{Missing baseline.}
A limitation shared by most LLM stylometry work, including ours,
is the absence of a human rewriter baseline.
\citet{koppel2009} show that even human authors exhibit significant
intra-author variance across genres and time.
It is unknown whether a human asked to ``neutralise'' then
``simplify'' the same text would produce per-text shift correlations
comparable to our LLM measurements.
Without this baseline, the null per-text correlations we observe
cannot be definitively attributed to model-specific behaviour.
